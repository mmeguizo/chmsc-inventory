...
export class PayslipComponent implements OnInit {
    ...
    this.sgs.request('get', 'payroll/checkPayrollIfExist', {pid: this.pid, eid: this.eid}, async (res) => {
      ...
      if(res.success){
        ...
        this.form =  this.formBuilder.group({
          ...
          totalLates:               [res.data.late / 60, [Validators.required]],
  ...
  paySlip(status){
    ...
    if(status == true){
      ...
      this.taxableIncome = ((this.totalMinutes / 60 ) + this.approvedOT ) * this.curEmployment.hourlyRate;
      ...
      // if(this.approvedOT){
      //   this.taxableIncome =  (this.approvedOT * this.curEmployment.hourlyRate);
      // }
  ...
  create(){
    this.sgs.Modal({
      ...
      if(response){
        ...
        this.form.value.overtimeID = this.overtimeIDs

        console.log('create');
        console.log(this.overtimeIDs);
        console.log(this.overtimeIDs === undefined);


        // set the overtime data to paid

        if(this.overtimeIDs === undefined){
        ...
        this.sgs.request('put','overtime/updatePaid', {id  : this.overtimeIDs}, async (res) => {
          if(res.success) {
                  this.sgs.request('post', 'payroll/addPayslip', {
                  eid: this.eid,
                  // loansInfo: [],
