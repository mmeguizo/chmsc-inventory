...
export class EmployeesComponent implements OnInit {
  ...
  addEmployee(id){
    ...
    // )
    const activeModal = this.ngbModal.open(AddEmployeesComponent, { size: 'xl', container: 'nb-layout', windowClass: 'min_height' });
  ...
  update(data){
    this.sgs.request('get', 'employee/getEmpById', {id : data}, async(res) => {
      if(res.success){
        this.sgs.Modal(
          {data : res.data[0]},
          {component : UpdateEmployeeComponent, size : 'xl'}
        )
      }
    })
