...
export class WorkingDaysComponent implements OnInit {
	...
	editWorking() {
		console.log(this.id);
		const activeModal = this.ngbModal.open(EditWorkingDaysComponent, { size: 'lg', container: 'nb-layout', windowClass: 'min_height' });
		activeModal.componentInstance.id = this.id;
	}

	addWorkingDays() {
		const activeModal = this.ngbModal.open(AddWorkingDaysComponent, { size: 'lg', container: 'nb-layout', windowClass: 'min_height' });
	}
