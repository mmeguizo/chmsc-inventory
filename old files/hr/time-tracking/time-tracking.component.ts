...
export class TimeTrackingComponent implements OnInit {
	...
	constructor(public sgs: SharedGlobalService, public ngbModal: NgbModal, public dp: DatePipe, public rd: ReverseDate) {
	...
	getTodayAttendance() {
		...
		this.sgs.request('get', 'day/getDays', {}, async (res) => {
			if (res.success) {
				...
				this.sgs.request('get', 'attendance/getTodayAttendance', { date: this.toDate }, async (res) => {
					let lateIn = 0;
					let lateOut = 0;
	...
	getAllPayrolledUsers(data) {
		this.sgs.request('get', 'user/getAllPayrolledUsers', { data: data }, (res) => {
			if (res.success) {
				this.noData = '';
				this.loading = false;
				res.data.map((a) => this.data.push(a));
			} else {
	...
	timeSheet(id) {
		...
		this.sgs.gothere('hr/timeSheet', id);
	}
}
