...
@ViewChild("search", { static: false }) nameField: ElementRef;
  ...
  getAllOvertime(){
    this.sgs.request('get', 'overtime/getAllOvertime', {}, (res) => {
  ...
  deleteOT(id){
    ...
    this.sgs.Modal( {
      header : ` Approve or Reject OT for Request ID ${id}?`,
      content : `Click Yes/Approve or No/Reject `,
      type : `confirmation`
    },{size : 'sm'} ).confirm.subscribe( response =>{
      response && ( this.sgs.request('put','overtime/deleteOT', {id  : id}, async (res) => {
  ...
  approvedOT(info){
    ...
    this.sgs.Modal( {
      header : ` Approve or Reject OT for ${info.name}?`,
      content : `Click Yes/Approve or No/Reject `,
      type : `confirmation`
    },{size : 'sm'} ).confirm.subscribe(  response => {
      if (response) {
                switch (info.approved) {
                  case 'approved':
                          this.sgs.showToaster('info', 'No need to do it', 'Already Approved', 3000, 'bottom-right');
                  break;

                   case '"rejected"':
                              this.sgs.request('put','overtime/approvedOT', {id  : info.id}, async (res) => {
                                res.success && [this.sgs.showToaster('success', 'Approved OT Successfully.', 'Success', 3000, 'bottom-right'), this.closeModal()]
                                !res.success && [this.sgs.showToaster('warning', 'Unable to approved.', 'Ooops', 3000, 'bottom-right'), this.closeModal()]
                              })
                  break;
                  default:    this.sgs.request('put','overtime/approvedOT', {id  : info.id}, async (res) => {
                    res.success && [this.sgs.showToaster('success', 'Approved OT Successfully.', 'Success', 3000, 'bottom-right'), this.closeModal()]
                    !res.success && [this.sgs.showToaster('warning', 'Unable to approved.', 'Ooops', 3000, 'bottom-right'), this.closeModal()]
                  })
                  break;
                ...
                switch (info.approved) {
                    case 'rejected':
                      this.sgs.showToaster('info', 'No need to do it', 'Already Rejected', 3000, 'bottom-right');
                    break;
                    default:   this.sgs.request('put','overtime/rejectOT', {id  : info.id}, async (res) => {
                                res.success && [this.sgs.showToaster('success', 'OT Rejected', 'Success', 3000, 'bottom-right'), this.closeModal()]
                                !res.success && [this.sgs.showToaster('warning', 'Unable to Rejected.', 'Ooops', 3000, 'bottom-right'), this.closeModal()]
                              });
                    break;
                }
      }
